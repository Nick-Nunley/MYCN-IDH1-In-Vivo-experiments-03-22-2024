...
def load_init_data(filepath: str, sep: str = ',', lines_to_skip: int = 3):
...
def load_scaling_vector(input_path: str):
...
def scale_protein_volumes(input_dict: dict, scaling_vector: list):
...
def write_to_csv(dictionary: dict, filename: str, columns = None):
...
def main_function(input_filepath: str, norm_path: str, output_path: str):
    """Main function to wrap everything into"""
    data_dict = load_init_data(filepath = input_filepath)
    scaling_vector = load_scaling_vector(input_path = norm_path)
    data_dict = scale_protein_volumes(
